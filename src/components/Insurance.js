...
const Insurance = ({insurance, updateInsurance}) =>{
    ...
    const updateFullWageHandler = (event) =>{
        const name = event.target.name;
        updateInsurance({
            ...insurance,
            ...
            <div className={classes.content}>                
                <div>
                    ...
                    <span>
                        ...
                        <input type='radio' name="fullWage" checked={insurance.fullWage} onChange={updateFullWageHandler}/>
                    </span>
                    <span>
                        <label>other </label>
                        <input type='radio' name ="fullWage" checked={!insurance.fullWage} onChange={updateFullWageHandler}/>
                ...
                <div style={{marginTop:'10px'}}>
                    ...
                    {initialRegion.map((item) =>{
                        return (
                            <span key={item.id}>
                                <input type="radio" name="region" checked={insurance.region.id === item.id} onChange={()=>{
                                changeInputHandler(item.id)
                                }}></input>
